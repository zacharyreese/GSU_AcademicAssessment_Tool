...
<body>
	...
	<div class="">
		<h2>All Mission Objectives</h2> <br><hr><br>
    <span id='success'> ${(success)!""} </span>
    <form action="mainServlet" method="Post">
        <div>
            <ol>
                <#list missionObjectives as obj>
                    <li>${obj.description}<br>
                    Status: <span id="isActive"><#if obj.isCurrentlyUsed == "1">Active<#else>Inactive</#if></span>
                    </li><br>
                   
                </#list>
            </ol>
        </div>
        <input type="submit" name="gotoInsertNewMissionObjective" value="Add new Mission Objective" class='button'>         
    </form>
